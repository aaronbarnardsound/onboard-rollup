import { c as createCommonjsModule, g as getCjsExportFromNamespace, b as commonjsGlobal, u as unwrapExports } from './main-17127497.js';
import { E as EventEmitter } from './events-767f970f.js';
import { b as bufferEs6, B as Buffer$h, i as isBuffer$1, g as global } from './buffer-es6-e564b388.js';
import { a as require$$0$1, b as browser$1$1, s as stringDecoder, S as Stream, i as inherits, c as isFunction$1, d as inspect$2, e as isDate, f as isRegExp, g as isPrimitive, h as isError } from './__node-resolve:empty-4a191708.js';
...
var isBuffer = function (value, message) {
  if (!Buffer$h.isBuffer(value)) throw TypeError(message)
};
...
var EC_PRIVKEY_EXPORT_DER_COMPRESSED = Buffer$1.from([
  // begin
...
var EC_PRIVKEY_EXPORT_DER_UNCOMPRESSED = Buffer$1.from([
  // begin
...
var privateKeyExport = function (privateKey, publicKey, compressed) {
  var result = Buffer$1.from(compressed ? EC_PRIVKEY_EXPORT_DER_COMPRESSED : EC_PRIVKEY_EXPORT_DER_UNCOMPRESSED);
...
var signatureExport = function (sigObj) {
  var r = Buffer$1.concat([Buffer$1.from([0]), sigObj.r]);
  for (var lenR = 33, posR = 0; lenR > 1 && r[posR] === 0x00 && !(r[posR + 1] & 0x80); --lenR, ++posR);

  var s = Buffer$1.concat([Buffer$1.from([0]), sigObj.s]);
...
var signatureImport = function (sig) {
  var r = Buffer$1.alloc(32, 0);
  var s = Buffer$1.alloc(32, 0);
...
exports.ecdh = function (publicKey, privateKey) {
  ...
  return browser$1('sha256').update(shared).digest()
};
...
function encode(input) {
    if (Array.isArray(input)) {
        ...
        var buf = Buffer$h.concat(output);
        return Buffer$h.concat([encodeLength(buf.length, 192), buf]);
    }
    else {
        var inputBuf = toBuffer(input);
        return inputBuf.length === 1 && inputBuf[0] < 128
            ? inputBuf
            : Buffer$h.concat([encodeLength(inputBuf.length, 128), inputBuf]);
    }
}
...
function encodeLength(len, offset) {
    if (len < 56) {
        return Buffer$h.from([len + offset]);
    }
    else {
        var hexLength = intToHex(len);
        var lLength = hexLength.length / 2;
        var firstByte = intToHex(offset + 55 + lLength);
        return Buffer$h.from(firstByte + hexLength, 'hex');
    }
}
...
function intToBuffer(integer) {
    ...
    return Buffer$h.from(hex, 'hex');
}
/** Transform anything into a Buffer */
function toBuffer(v) {
    if (!Buffer$h.isBuffer(v)) {
        if (typeof v === 'string') {
            if (isHexPrefixed(v)) {
                return Buffer$h.from(padToEven(stripHexPrefix(v)), 'hex');
            }
            else {
                return Buffer$h.from(v);
            }
        }
            ...
            return Buffer$h.from(v.toArray());
        }
        else {
...
function intToBuffer(i) {
  ...
  return new Buffer$h(padToEven(hex.slice(2)), 'hex');
}
...
function getBinarySize(str) {
  ...
  return Buffer$h.byteLength(str, 'utf8');
}
 ...
 */
function toUtf8(hex) {
  var bufferValue = new Buffer$h(padToEven(src$1(hex).replace(/^0+|0+$/g, '')), 'hex');
...
function fromUtf8(stringValue) {
  var str = new Buffer$h(stringValue, 'utf8');
...
export { assert$b as a, browser$1 as b, bn as c, dist as d, elliptic$1 as e, src$1 as f, inherits_browser as i, lib$1 as l, safeBuffer as s };
